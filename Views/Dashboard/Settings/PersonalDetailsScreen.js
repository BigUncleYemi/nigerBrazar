  ...
  const handleRes = async res => {
    ...
    if (res.didCancel) {
      ...
      const name =
        ...
        res.assets[0].uri.substr(res.assets[0].uri.lastIndexOf('/') + 1);
      const source = {
        uri,
        type,
        name,
      ...
      //console.log(resp, 'outdd');
      updateUserProfile.mutate({photoUrl: resp.secure_url});
    }
  };
        ...
        <VStack p={8}>
          ...
          <FormInput
            ...
            Component={({value, onChangeText, onBlur}) => {
              const fecthText = React.useCallback(text => {
              ...
              return (
                <PhoneInput
                  ...
                  defaultValue={
                    ...
                    parsePhoneNumber(value).nationalNumber
                  }
                  layout="first"
          ...
          <FormInput
            ...
            label="Language"
            rules={{required: 'Field is required'}}
            defaultValue={user?.profile?.language}
            Component={({value, onChangeText, onBlur}) => {
              return (
                <Select
                  selectedValue={value}
          ...
          <Button
            ...
            mt={5}
            isLoading={updateUserProfile.status === 'loading'}
            isDisabled={updateUserProfile.status === 'loading'}
